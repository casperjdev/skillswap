...
const wordCount = computed(() => content.value.trim().split(/\s+/).filter(w => w.length > 0).length);
</script>
...
<template>
  ...
  <Card v-else
    ...
    <header class="rounded-md border-b border-neutral-800 z-20 bg-neutral-900/50 md:bg-transparent">
      ...
      <div :class="['transition-all duration-300 overflow-hidden md:overflow-visible', isMobileSettingsOpen ? 'max-h-[700px] opacity-100 border-t border-neutral-800 md:border-t-0' : 'max-h-0 opacity-0 md:max-h-none md:opacity-100']">
    ...
    <!-- Enhanced Toolbar -->
    <div class="border-b border-neutral-800 flex items-center justify-between px-3 py-2 shrink-0 bg-neutral-900/30 overflow-x-auto">
      <div class="flex items-center gap-1 min-w-max">
        <button @mousedown.prevent @click="formatBold" title="Bold (Ctrl+B)" class="group relative w-8 h-8 flex items-center justify-center rounded-md text-neutral-400 hover:bg-neutral-700 hover:text-emerald-400 font-bold text-sm transition-all active:scale-95">
          <span class="font-bold">B</span>
        </button>
        <button @mousedown.prevent @click="formatItalic" title="Italic (Ctrl+I)" class="group relative w-8 h-8 flex items-center justify-center rounded-md text-neutral-400 hover:bg-neutral-700 hover:text-emerald-400 italic font-serif text-sm transition-all active:scale-95">
          <span class="italic">I</span>
        </button>

        <div class="w-px h-5 bg-neutral-700 mx-1"></div>

        <button @mousedown.prevent @click="formatH2" title="Heading 1" class="w-8 h-8 flex items-center justify-center rounded-md text-neutral-400 hover:bg-neutral-700 hover:text-emerald-400 text-xs font-bold transition-all active:scale-95">H1</button>
        <button @mousedown.prevent @click="formatH3" title="Heading 2" class="w-8 h-8 flex items-center justify-center rounded-md text-neutral-400 hover:bg-neutral-700 hover:text-emerald-400 text-xs font-bold transition-all active:scale-95">H2</button>

        <div class="w-px h-5 bg-neutral-700 mx-1"></div>

        <button @mousedown.prevent @click="formatList" title="List" class="w-8 h-8 flex items-center justify-center rounded-md text-neutral-400 hover:bg-neutral-700 hover:text-emerald-400 transition-all active:scale-95">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"></path></svg>
        </button>

        <button @mousedown.prevent @click="formatCode" title="Code" class="w-8 h-8 flex items-center justify-center rounded-md text-neutral-400 hover:bg-neutral-700 hover:text-emerald-400 transition-all active:scale-95">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4"></path></svg>
        </button>

        <button @mousedown.prevent @click="formatLink" title="Link" class="w-8 h-8 flex items-center justify-center rounded-md text-neutral-400 hover:bg-neutral-700 hover:text-emerald-400 transition-all active:scale-95">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1"></path></svg>
        </button>

        <button @mousedown.prevent @click="formatBlockquote" title="Quote" class="w-8 h-8 flex items-center justify-center rounded-md text-neutral-400 hover:bg-neutral-700 hover:text-emerald-400 transition-all active:scale-95">
      ...
      <div class="flex items-center gap-3 ml-3 shrink-0">
        ...
        <div class="flex rounded-lg p-0.5 md:hidden border border-neutral-700 bg-neutral-800/50">
          <button @click="activeTab = 'write'" :class="['px-3 py-1.5 text-[10px] font-bold uppercase transition-all rounded', activeTab === 'write' ? 'bg-neutral-700 text-white shadow-sm' : 'text-neutral-500 hover:text-neutral-300']">Edit</button>
          <button @click="activeTab = 'preview'" :class="['px-3 py-1.5 text-[10px] font-bold uppercase transition-all rounded', activeTab === 'preview' ? 'bg-emerald-600 text-white shadow-sm' : 'text-neutral-500 hover:text-neutral-300']">Preview</button>
        </div>
      </div>
    </div>
    ...
    <div class="flex-1 flex overflow-hidden relative bg-neutral-950/30">
      <!-- Editor Pane -->
      <div :class="['flex-col md:flex w-full md:w-1/2 h-full relative', activeTab === 'write' ? 'flex' : 'hidden']">
        <div class="absolute top-0 left-0 right-0 h-20 bg-gradient-to-b from-neutral-900/20 to-transparent pointer-events-none z-10"></div>
        <textarea
            ref="textareaRef"
            v-model="content"
...
And much more!"
      ...
      <!-- Divider -->
      <div class="hidden md:block w-px bg-gradient-to-b from-transparent via-neutral-700 to-transparent h-full shrink-0"></div>

      <!-- Enhanced Preview Pane -->
      <div :class="['w-full md:w-1/2 h-full overflow-y-auto custom-scrollbar bg-neutral-900/20', activeTab === 'preview' ? 'block' : 'hidden md:block']">
        <div class="min-h-full">
          <div class="max-w-3xl mx-auto px-6 md:px-12 py-8 md:py-12">

            <!-- Video Embed -->
            <div v-if="embedUrl" class="mb-10 group">
              <div class="rounded-2xl overflow-hidden shadow-2xl shadow-black/60 border border-neutral-800/50 aspect-video bg-black ring-1 ring-white/5 transform transition-transform group-hover:scale-[1.01]">
                <iframe
                    :src="embedUrl"
                    class="w-full h-full"
                    frameborder="0"
            ...
            <!-- Empty State -->
            <div v-if="!content.trim() && !embedUrl" class="flex flex-col items-center justify-center py-20 text-center">
              ...
              <p class="text-neutral-500 text-sm font-medium">Your content preview will appear here</p>
              <p class="text-neutral-600 text-xs mt-1">Start typing in the editor to see the magic ✨</p>
            </div>
          </div>
        </div>
      </div>
    ...
    >
      <div v-if="showToast" class="fixed bottom-6 left-1/2 -translate-x-1/2 bg-neutral-800 border border-emerald-500/30 text-white px-5 py-2.5 rounded-full shadow-[0_0_30px_rgba(16,185,129,0.2)] flex items-center gap-2.5 text-xs font-semibold z-50 backdrop-blur-sm">
...
.flex .text-[10px] {
  ...
  /* Reduce toast size */
  .fixed[role] { /* generic selector fallback; if role not present, toast still targeted by earlier class */
    font-size: 0.75rem;
  }

  /* Optionally reduce spacing a bit to fit small screens */
  .p-6 { padding: 0.75rem; }
  .md\:p-8 { padding: 0.75rem; }
