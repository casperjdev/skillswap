...
<template>
  ...
  >
    <div class="w-full my-2">
      ...
      <label v-else class="w-full h-40 sm:h-48 border-2 border-dashed border-white/40 rounded-md flex flex-col items-center justify-center cursor-pointer hover:border-white/60 transition-colors active:bg-white/5">
        <Icon name="lucide:upload" class="w-10 h-10 sm:w-8 sm:h-8 text-white/60 mb-2" />
        <span class="text-white/60 text-sm sm:text-base text-center px-4">Click and upload a cover image</span>
        <input
            type="file"
            accept="image/*"
