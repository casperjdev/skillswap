...
<template>
  ...
  >
    ...
    <div class="w-full my-2">
      ...
      />
      <div class="flex flex-nowrap overflow-x-scroll md:flex-wrap gap-2 mt-3 pb-2 scrollbar-thin scrollbar-thumb-white/20 scrollbar-track-transparent hover:scrollbar-thumb-white/40">
        <span
            v-for="(tag, index) in modelValue"
            :key="index"
            class="bg-white/20 max-md:text-2xs text-white px-3 py-1.5 sm:py-1 rounded-full text-sm sm:text-base flex items-center gap-2 h-fit whitespace-nowrap flex-shrink-0"
        >
          {{ tag }}
          <button
              @click="removeTag(index)"
              class="hover:text-red-300 min-w-[24px] min-h-[24px] flex items-center justify-center"
          >
